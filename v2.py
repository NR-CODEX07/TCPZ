...
def get_player_info(player_id):
    url = f"https://nr-codex-info.vercel.app/get?uid={player_id}"
    response = requests.get(url)    
    if response.status_code == 200:
        try:
            r = response.json()
            return {
                "Account Booyah Pass": f"{r.get('AccountInfo', {}).get('AccountBPBadges', 'N/A')}",
                "Account Create": f"{r.get('AccountInfo', {}).get('AccountCreateTime', 'N/A')}",
                   "Account Likes": f"{r.get('AccountInfo', {}).get('AccountLikes', 'N/A')}",
    "Name": f"{r.get('AccountInfo', {}).get('AccountName', 'N/A')}",
    "UID": f"{r.get('captainBasicInfo', {}).get('accountId', 'N/A')}",
    "Account Region": f"{r.get('AccountInfo', {}).get('AccountRegion', 'N/A')}",
}
...
def spam_requests(player_id):
    ...
    url = f"https://gamigo-admin-api.vercel.app/send_requests?uid={player_id}"
    try:
...
def newinfo(uid):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if "player_info" in data and "basicInfo" in data.get("player_info", {}):
                return {"status": "ok", "data": data["player_info"]}
            else:
...
def send_likes(uid):
    try:
        ...
        likes_api_response = requests.get(
            f"https://nr-codex-like-api2.vercel.app/like?uid={uid}&server_name=IND",
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Credits: [FFFFFF]GAMIGO [00FF00]YT!!
                """
            else:
...
Could not add likes.
This might be due to an invalid ID,
or you have reached your daily limit.

(Status: {api_json_response.get('status', 'N/A')})
...
def  generate_random_word():
    word_list = [
        ...
        "FAILED_STREAMER", "NILAY", "GAY", "YOUR_ASS_IS_PINK", "YOUR_PUSSY_IS_BEAUTIFUL",
        ...
        "LOSER", "FUCK_YOU", "FUCK_YOUR_MOM_SLANG", "FUCK_YOUR_MOM_SLANG2", "I_WILL_JERK_OFF_ON_YOU", "LICKER", " NILAY", "TELEGRAM:@NILAY_VII"
    ]
...
class FF_CLIENT(threading.Thread):
    ...
    # New function added from bot.zip/byte.py
    def join_teamcode(self, inv, room_id):
        ...
        if len(header_length_hex) == 2:
            ...
            raise ValueError("Packet header length error")
    ...
    def GenResponsMsg(self, Msg):
        ...
        1: 1,
        2: {
            1: 12991672692,#bot account id
            2: 3038748483,#clan id
            3: 1,
            4: str(Msg),
    ...
    def joinclanchat(self):
        fields = {
            1: 3,
            2: {
                1: 3038748483,#clan id
                2: 1,
                4: str("HWUtPMATROCb1tS5VWwaMg"),#clan key
            }
        }
        packet = create_protobuf_packet(fields)
        packet = packet.hex()
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""[C][B]
Welcome to [00ff00]GAMIGO'S BOT[ffffff]!
...
[FFB300][b][c]BOT BY : GAMIGO
            ...
            if "1200" in data.hex()[0:4] and b"/spm/" in data:
                try:
                    ...
                    response_message = ""
                    try:
                        api_url = f"https://gamigo-admin-api.vercel.app/send_requests?uid={player_id}"
...
####################################
            ...
            if "1200" in data.hex()[0:4] and b"/solo" in data:
                try:
                    ...
                    # Send leave squad packet
                    leave_packet = self.leave_s()
                    socket_client.send(leave_packet)
                    sleep(1) # Wait a moment for the leave to process
                    
                    # Send change mode to solo packet
                    solo_packet = self.changes(1) # Assuming '1' is the code for solo mode
                    socket_client.send(solo_packet)
                    
                    clients.send(self.GenResponsMsg("[C][B][00FF00]Left squad and switched to Solo mode."))
                except Exception as e:
                    print(f"Error in /solo command: {e}")
                    clients.send(self.GenResponsMsg("[C][B][FF0000]An error occurred in /solo command."))

            # <------- NEW /go Command Handler (from /start) ------->
            if "1200" in data.hex()[0:4] and b"/go" in data:
                try:
                    split_data = re.split(rb'/go', data)
                    command_parts = split_data[1].split(b'(')[0].decode().strip().split()

                    if not command_parts:
                        clients.send(self.GenResponsMsg("[C][B][FF0000]Please provide a team code. Usage: /go [TeamCode]"))
                        continue

                    team_code = command_parts[0]
                    spam_count = 15 # Default spam count

                    clients.send(self.GenResponsMsg(f"[C][B][FFA500]Joining lobby to force start..."))
                    
                    # Join the team using the provided code
                    self.join_teamcode(socket_client, team_code)
                    time.sleep(2)  # Wait to ensure successful join

                    clients.send(self.GenResponsMsg(f"[C][B][FF0000]Spamming start command {spam_count} times!"))
                    
                    # Spam the start game command
                    start_packet = self.start_autooo()
                    for _ in range(spam_count):
                        socket_client.send(start_packet)
                        time.sleep(0.1)

                    clients.send(self.GenResponsMsg(f"[C][B][00FF00]Force start process finished."))

                except Exception as e:
                    print(f"An error occurred in /go command: {e}")
                    clients.send(self.GenResponsMsg("[C][B][FF0000]An error occurred during the force start process."))
            
            #<------- NEW /lag Command Handler ------->
            if '1200' in data.hex()[0:4] and b'/lag' in data:
                try:
                    ...
                    if not command_parts:
                        clients.send(self.GenResponsMsg("[C][B][FF0000]Please provide a team code. Usage: /lag [TeamCode] [repeat]"))
                        continue
                    ...
                    if repeat_count > 5: 
                        repeat_count = 5 # Limit to 5 repeats to avoid issues

                    clients.send(self.GenResponsMsg(f"[C][B][FFA500]Starting lag process on {room_id}, repeating {repeat_count} time(s)."))
                    
                    for i in range(repeat_count):
                         clients.send(self.GenResponsMsg(f"[C][B][FFA500]Running batch {i + 1} of {repeat_count}..."))
                         for _ in range(11111): # Spam 30 times per batch
                            self.join_teamcode(socket_client, room_id)
                            time.sleep(0.001) 
                            leavee = self.leave_s()
                            socket_client.send(leavee)
                            time.sleep(0.001)
                    
                    clients.send(self.GenResponsMsg(f"[C][B][00FF00]Lag process finished on {room_id}!"))

                except Exception as e:
                    print(f"Error in /lag command: {e}")
                    try:
                        clients.send(self.GenResponsMsg("[C][B][FF0000]An error occurred during the lag process."))
                    except: pass
            # <------- End of New Command Handlers ------->
...
[FFFFFF][B]Check player’s server/region:
[1E90FF]/🗿region/104[c]145[c]933[c]49

[FFFFFF][B]Add 100 likes to an account:  
[FF69B4]/🗿like/104[c]145[c]933[c]49

[FFFFFF][B]Lag a team with a team code:
[b3ff00]/🗿lag [TeamCode] [repeat 1-5]

[FFFFFF][B]Force start a match:
[FFA500]/🗿go [TeamCode]

[FFFFFF][B]Leave squad & return to solo:
[00ffdd]/🗿solo

[FFFFFF][B]Chat with ChatGPT:  
[9370DB]/🗿ai [your text here]

[FFFFFF][B]OWNER: GAMIGO YT
